...
- - User can override via environment variable before running tests - This variable is used by wrapper scripts to locate
  the Rust canonical binary
...
- - Execution: Uses Pester v5+ framework - Verbosity: 'Detailed' (shows test names and results)
...
- - Perl 5.10+ required - prove (typically included with Perl, part of Test::Harness) - Fallback: Direct perl execution
  if prove not found
...
- - Linux (primary CI platform) - macOS (local development) - Windows (Git Bash, WSL for Bash runners; native PowerShell
  for RunTests.ps1)
...
1. 1. **Set SAFE_RUN_BIN environment variable** - Detect repository root (2 levels up from wrapper directory)
   - Default to `{repo_root}/rust/target/release/safe-run`
   - Allow override via pre-existing `SAFE_RUN_BIN` environment variable

2. 2. **Use language-specific test framework** - Python: unittest (matches existing run-tests.sh) - Perl: Test::Harness
   (prove) or direct perl execution - PowerShell: Pester (already implemented)

3. 3. **Discover tests with correct pattern**
...
4. 4. **Exit with standard codes** - 0 = all tests passed - 1 = one or more tests failed - 2 = prerequisites not met
   (optional, recommended for clarity)

5. 5. **Run from wrapper directory OR repo root**
...
6. 6. **Provide actionable error messages** - Missing prerequisites: Clear installation instructions - No tests found:
   Explicit error message - Test failures: Delegate to test framework output

7. 7. **Match output verbosity** - Show individual test names and results - Match style of existing Bash runner for that
   language - Helps with debugging in CI logs
...
3. 3. Language-native runner forwards exit code 4. Language-native runner provides language-specific error handling
   (e.g., check for bash availability)

**Future migration path (documented in future-work.md):**

- - Option to migrate to fully native implementations later - Pros: No bash dependency, language-specific test discovery
  optimizations - Cons: More code to maintain, risk of drift - Decision deferred until there's a strong reason to
  migrate
...
- - Each wrapper test job runs BOTH:
  ...
  2. 2. New language-native runner - Both must pass for CI to pass
...
1. 1. ✅ All three language-native runners exist:
...
2. 2. ✅ All runners pass lint checks: - Python: flake8, black, pylint - Perl: Perl::Critic - PowerShell:
   PSScriptAnalyzer

3. 3. ✅ All runners are functionally equivalent: - Same exit codes for pass/fail/error - Same environment setup
   (SAFE_RUN_BIN) - Same test discovery behavior - Work from both wrapper directory and repo root

4. 4. ✅ CI workflows updated: - Each language runs both Bash and native runner - Both must pass
...
6. 6. ✅ Full test suite passes: - All wrapper tests pass (bash/perl/powershell/python3) - Conformance tests pass - CI is
   green
...
- - **2025-12-29:** Created as part of Phase 5, Item 5.1.1-5.1.2
  ...
  - - Defined parity requirements for language-native runners - Established thin wrapper implementation strategy -
