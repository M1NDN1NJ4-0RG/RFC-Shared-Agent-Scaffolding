...
- - If lint/docstring violations exist, the job **exits non-zero** and the job shows **Failed** (red). - Only true clean
  runs should be green.
...
touching ONLY Markdown → only Detect + Consolidate run (no language jobs)
...
- - We can force-run all language jobs deterministically for verification - Fixtures exist to prove failure paths are
  exercised
