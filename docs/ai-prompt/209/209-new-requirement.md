...
- - - Create a dedicated test directory, e.g. `tools/repo_lint/tests_bootstrapper/` or `tests/bootstrapper/`. - Use a
  test runner that already exists in the repo (prefer `pytest`). - - Use fixtures that create a temporary fake repo
  layout where needed. - For external commands, use **mocking** (e.g., patch `PATH` to point at stub executables) so
  tests do not depend on the host machine. - - Add CI job(s) to run these tests.
