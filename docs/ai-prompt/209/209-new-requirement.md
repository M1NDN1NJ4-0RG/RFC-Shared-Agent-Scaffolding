...
2) Tests must be **repeatable, deterministic, and runnable in CI** (non-interactive, no hanging).
3) Tests must cover **success paths AND failure paths**, including clear assertions on:
   - exit codes
   - stdout/stderr content (key lines)
   - created files/dirs (venv dirs, caches, etc.)
   - PATH / environment effects (only where intended)

## Minimum test coverage (MANDATORY)
### A) Repo root discovery
- Running from repo root succeeds
- Running from nested subdir succeeds
- Running outside repo fails with expected exit code and message
...
- “missing tool” path triggers installation attempt OR explicit failure message (depending on environment capability)
Tools list:
- rgrep (or fallback behavior)
- black, pylint, pytest, ruff, yamllint
- shellcheck, shfmt
...
- Bootstrapper must run `repo-cli check --ci` (or the configured gate) and:
  - Pass scenario: exits 0
  - Fail scenario: exits non-zero with actionable output
...
1) New test files + fixtures + stubs
2) CI wired to run them
3) In your PR comment, include:
   - the exact test command(s)
   - a list of test cases added
