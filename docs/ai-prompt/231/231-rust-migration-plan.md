...
1. 1. 1. **Performance:** - 30-50% faster than Bash (via parallelism) - Measured in CI over 20 runs
...
1. 1. 1. Should we support user-defined installer plugins (future)? 2. How to handle version downgrades (disallow?
   warn?)? 3. Should cache be per-user or per-repo? 4. How aggressive should parallel downloads be (respect bandwidth)?
