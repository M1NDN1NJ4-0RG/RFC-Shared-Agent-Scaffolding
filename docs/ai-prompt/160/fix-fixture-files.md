...
~~~
4) Sanity check the diff file is non-empty:
~~~bash
...
~~~
3) Verify the fixture is restored to canonical (use `git diff` to confirm it matches the expected “bad” content state you authored).
4) If `git apply` fails:
...
4) Stop ONLY after the repo is clean and resumable:
~~~bash
git status
~~~
