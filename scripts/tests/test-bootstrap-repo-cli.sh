#!/usr/bin/env bash
#
# test-bootstrap-repo-cli.sh - Tests for bootstrap-repo-cli.sh
#
# DESCRIPTION:
#   Validates that bootstrap-repo-cli.sh correctly bootstraps the repo-lint
...
#
# shellcheck disable=SC2317  # Test helper functions are called indirectly via 't' runner
...
test_find_repo_root_git() {
	...
	(
		...
		mkdir -p .git
		cd subdir/nested
		# Extract the function from the script (from function start to closing brace)
		eval "$(sed -n '/^find_repo_root()/,/^}/p' "$BOOTSTRAP_SCRIPT")"
		local result
