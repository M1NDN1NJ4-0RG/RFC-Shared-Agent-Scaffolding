...
pub fn write_activation_script(repo_root: &Path, venv_path: &Path) -> Result<()> {
    ...
    let script_content = format!(
        r#"#!/usr/bin/env bash
#
# activate.sh - Bootstrap Environment Activation Script
#
# DESCRIPTION:
#   Activates the development environment created by the bootstrap-repo-cli tool.
#   This script configures the Python virtual environment and Perl toolchain paths
#   to make all installed tools available in the current shell session.
#
#   This script MUST be sourced, not executed directly, to modify the current
...
#     PERL5LIB  (optional) - Existing Perl library paths are preserved and appended
#
# OUTPUTS:
#   Exit Codes:
#     1    Script was executed instead of sourced (error)
#
#   Environment Variables:
#     PATH                - Prepended with Python venv bin and Perl tools bin
#     PERL5LIB            - Set to Perl module library path
#     VIRTUAL_ENV         - Set by Python venv activation
#     BOOTSTRAP_ACTIVATED - Marker flag set to "1"
#
#   Stdout:
#     Status messages indicating activation progress
#
# EXAMPLES:
...
#
#   # Verify activation
#   echo $BOOTSTRAP_ACTIVATED  # Should print "1"
#
# NOTE:
#   Auto-generated by bootstrap-repo-cli. Do not edit manually.
#   To regenerate, run: bootstrap-repo-cli install --profile dev
