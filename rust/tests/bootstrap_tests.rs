...
mod exit_code_tests {
    ...
    fn test_exit_codes_in_valid_range() {
        ...
        for code in exit_codes {
            assert!(code >= 0 && code < 128, "Exit code {} must be in range 0-127", code);
        }
    }
...
mod path_construction_tests {
    ...
    fn test_tools_repo_cli_path() {
        ...
        assert!(pyproject.to_str().unwrap().ends_with("tools/repo_cli/pyproject.toml"));
    }
...
mod error_message_tests {
    ...
    fn test_repo_lint_install_error() {
        let msg = "repo-lint install failed. Missing tools are BLOCKER. Install missing tools and rerun.";
