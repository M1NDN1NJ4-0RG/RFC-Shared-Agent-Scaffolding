...
write_file(File::Spec->catfile($fail_dir, "a fail.txt"), "A\n");
write_file(File::Spec->catfile($fail_dir, "b-fail.txt"), "B\n");
...
# 1) --all move, no compression
{
  ...
  ok(!-e File::Spec->catfile($fail_dir, "a fail.txt"), "removed from FAIL-LOGS");
  ok(-e File::Spec->catfile($arch_dir, "a fail.txt"), "moved to archive (spaces preserved)");
  ok(-e File::Spec->catfile($arch_dir, "b-fail.txt"), "moved second");
}
...
# 2) no-clobber: if target exists, should not overwrite, should create suffixed file
{
  ...
  make_path($fail_dir);
  write_file(File::Spec->catfile($fail_dir, "b-fail.txt"), "NEW\n");
  # Create existing file in archive
  write_file(File::Spec->catfile($arch_dir, "b-fail.txt"), "OLD\n");

  my $r = run_cmd(["perl", $safe_archive, "--all"], cwd=>$sandbox, env=>{ SAFE_FAIL_DIR=>$fail_dir, SAFE_ARCHIVE_DIR=>$arch_dir, SAFE_ARCHIVE_COMPRESS=>"none" });
  is($r->{exit}, 0, "archive exit 0 even with collisions");

  my $old = slurp(File::Spec->catfile($arch_dir, "b-fail.txt"));
  is($old, "OLD\n", "existing archive file not overwritten");

  # Find new file with suffix
  opendir my $dh, $arch_dir or die $!;
  my @cands = grep { /^b-fail\.\d+\.txt$/ } readdir($dh);
  closedir $dh;
